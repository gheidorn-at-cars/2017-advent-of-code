defmodule Day2 do
  def checksum([]), do: 0
  def checksum([_]), do: 0
  def checksum(rows), do: row_diff(rows, 0)

  def row_diff([current | last], acc) do
    row_diff(last, acc + Enum.max(current) - Enum.min(current))
  end

  def row_diff([], acc) do
    # IO.puts("sum_equal([], _previous, #{acc})")
    acc
  end
end
...
defmodule Day2Test do
  ...
  test "regular cases" do
    assert checksum(['5195', '753', '2468']) == 18
    assert checksum(['1234', '9876', '105']) == 11
    assert checksum(['1111', '2222', '3333']) == 0

    assert checksum([[50, 100, 150], [444, 555, 666]]) == 322
  end
end
...
IO.inspect(Day2.checksum(input))
